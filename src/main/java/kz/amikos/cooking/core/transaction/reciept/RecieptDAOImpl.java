...
public class RecieptDAOImpl extends JdbcDaoSupport implements RecieptDAO{
	...
	private String SELECT_RECIEPT_DAO = "select username, reciept_name, reciept_description from user_reciepts where username=";
	...
	public List<Reciept> getUserReciepts(User user) {
		return getJdbcTemplate().query(SELECT_RECIEPT_DAO + "'" + user.getUsername() +"'", new RowMapper<Reciept>() {
